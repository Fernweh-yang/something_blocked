...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_ema = self.ema(x) # [B, C, H, W] -> [B, C, H, W]
        # 将SENet的输出重新调整为原始的维度
        x_ema = x_ema.permute(0, 2, 3, 1).view(B, patch_size * patch_size, D)  # [B, H,w,c] -> [B, 196, D]
        # 重新添加 cls token
        cls_token = self.cls_token.expand(B, -1, -1)  # [B, 1, D]
        x = torch.cat((cls_token, x_ema), dim=1)  # [B, 1, D] + [B, 196, D] -> [B, 197, D]   32, 197, 768
        # 到此为止
